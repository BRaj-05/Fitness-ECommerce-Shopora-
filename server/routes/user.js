...
function normalizeAddresses(addresses = [], defaultAddressId) {
  if (!Array.isArray(addresses)) return { addresses: [], defaultAddressId: undefined };

  const cleaned = addresses.map((address) => ({
    id: trimValue(address.id) || crypto.randomUUID(),
    label: trimValue(address.label) || "Home",
    line1: trimValue(address.line1) || "",
    line2: trimValue(address.line2) || "",
  ...
  const requestedDefault = trimValue(defaultAddressId);
  const validDefault = cleaned.some((address) => address.id === requestedDefault)
    ? requestedDefault
    : cleaned[0]?.id;

  return {
    addresses: cleaned,
    defaultAddressId: validDefault,
  };
}
...
router.put("/profile/:userId", verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const update = {};
    const allowed = ["name", "phone", "defaultAddressId", "photoURL"];
    for (const k of allowed) {
      if (req.body[k] !== undefined) update[k] = trimValue(req.body[k]);
    }
    ...
    if (req.body.addresses !== undefined) {
      ...
      update.addresses = normalized.addresses;
      update.defaultAddressId = normalized.defaultAddressId;
    }
