  ...
  return (
    ...
    >
      {photoURL ? (
        ...
        <span
          style={{ fontFamily: "Manrope, Inter, sans-serif", fontSize: size * 0.36 }}
...
const [rewardsError, setRewardsError] = useState("");
  ...
  useEffect(() => { document.title = "Profile | Shopora"; }, []);

  const reloadProfile = useCallback(async (user = auth.currentUser) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setRewardsError(error.message || "Could not load Shopora Rewards");
    } finally {
  ...
  const saveProfilePatch = async (payload) => {
    const user = auth.currentUser;
    if (!user) return navigate("/auth");
  ...
  const removeAddress = async (id) => {
    ...
    try {
      ...
      await saveProfilePatch({
        addresses,
        defaultAddressId: profile.defaultAddressId === id ? undefined : profile.defaultAddressId,
      });
      setToast("Address removed");
    } catch (error) {
      setAddressError(error.message || "Unable to remove address");
    } finally {
  ...
  const saveEditingAddress = async () => {
    ...
    try {
      const cleanAddress = Object.fromEntries(
        Object.entries(editingAddress).map(([key, value]) => [
          key,
          typeof value === "string" ? value.trim() : value,
        ]),
      );

      if (!cleanAddress.line1 || !cleanAddress.city || !cleanAddress.state || !cleanAddress.zip || !cleanAddress.country) {
        throw new Error("Complete the required shipping address fields");
      }

      const exists = profile.addresses.find((a) => a.id === cleanAddress.id);
      const addresses = exists
        ? profile.addresses.map((a) => (a.id === cleanAddress.id ? cleanAddress : a))
        : [...profile.addresses, cleanAddress];

      await saveProfilePatch({
        addresses,
        defaultAddressId: profile.defaultAddressId || cleanAddress.id,
      });
  ...
  { id: "rewards", label: "Shopora Rewards" },
    ...
    <div className="min-h-screen bg-stone-50" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-10 pb-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-5 mb-6">
            ...
            <div className="flex-1">
              ...
              <h1 style={{ fontFamily: "Manrope, Inter, sans-serif" }} className="text-3xl text-stone-900 leading-tight">
              ...
              <div className="space-y-3">
                {profile.addresses.map((a) => (
                  <AddressCard
                    ...
                    onSetDefault={() =>
                      saveProfilePatch({ defaultAddressId: a.id })
                        .then(() => setToast("Default address updated"))
                        .catch((error) => setAddressError(error.message || "Unable to update default address"))
                    }
                  />
                ))}
                ...
                <div className="pt-2">
                  ...
                  >
                    {saving || addressSaving ? "Saving..." : "Save Addresses"}
                  </button>
                </div>
              </div>
            )}
        ...
        {activeTab === "orders" && (
          <div>
            ...
            {loadingOrders ? (
              ...
              <div className="space-y-3">
                {orders.map((order) => (
                  <div key={order._id} className="shopora-premium-card rounded-3xl p-5 transition-all">
                              ...
                              state: {
                                ...
                                address:
                                  profile.addresses?.find(
                                    (item) => item.id === profile.defaultAddressId,
                                  ) || profile.addresses?.[0] || null,
                              },
                            })
                          }
                          className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 transition hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-600 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-indigo-500/10"
                        >
                          Invoice
                        </button>
      ...
      {editingAddress && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h3 style={{ fontFamily: "Manrope, Inter, sans-serif" }} className="text-xl text-stone-900">
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="flex-1 bg-stone-900 text-white text-sm py-3 rounded-full hover:bg-stone-700 transition-colors disabled:opacity-60"
              >
...
const ShoporaRewardsTab = ({ rewardsData, rewardsLoading, rewardsError }) => {
  if (rewardsLoading) {
    return (
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-sm font-medium text-stone-500">
              Current Shopora Rewards Points
            </p>
        ...
        {transactions.length === 0 ? (
          <div className="mt-6 rounded-2xl border border-dashed border-stone-300 bg-stone-50 p-8 text-center">
            ...
            <p className="mt-2 text-sm text-stone-600">
              ...
              earning Shopora Rewards points.
            </p>
          </div>
        ) : (
