...
The first-purchase email feature sends a welcome email to users after they successfully complete their first paid order on Shopora. The email is sent automatically from the backend, and email sending failures do not block the payment success flow.
...
2. **`server/services/emailTemplates.js`** - Email template generator
   ...
   - Includes Shopora branding and feature highlights
...
**Subject:** "Thank You for Choosing Shopora"

**Preview Text:** "Your first Shopora order is confirmed. Explore plans, workout tracking, nearby fitness centers, and more."

**Email Highlights:**
- Shopora branding and personalized greeting
...
**Steps:**
1. Log in to Shopora with a test user (Firebase auth)
2. Add items to cart
...
- Check server logs for:
  ...
  ```
- Check the user's email inbox for the Shopora welcome email
...
3. **Use environment-specific `SMTP_FROM`:**
   - Dev: `noreply-dev@Shopora.com`
   - Prod: `noreply@Shopora.com`
